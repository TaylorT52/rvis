...
where
    ...
    B: Sum3<T> + HasStorage<T, { D0 * (D1 * D2) }> + HasStorage<T, { D1 * D2 }> + HasStorage<T, { D0 * D2 }> + HasStorage<T, { D0 * D1 }>,
    [(); D0 * (D1 * D2)]:,
{
    #[inline]
...
where
    T: Float + Default,
    B: Mean3<T> + HasStorage<T, { D0 * (D1 * D2) }> + HasStorage<T, { D1 * D2 }> + HasStorage<T, { D0 * D2 }> + HasStorage<T, { D0 * D1 }>,
    [(); D0 * (D1 * D2)]:,
{
    #[inline]
