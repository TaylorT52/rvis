    ...
    T: Copy + Default + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + Div<Output = T>,
{
    ...
    #[inline]
    fn storage_uninit(len: usize) -> Self::Storage {
        vec![T::default(); len]
    }

    #[inline]
    fn as_slice(s: &Self::Storage) -> &[T] {
        s
    }

    #[inline]
    fn as_mut_slice(s: &mut Self::Storage) -> &mut [T] {
        s
    }
    ...
    fn add_scalar(lhs: &Self::Storage, rhs: T, out: &mut Self::Storage) {
        for (o, a) in out.iter_mut().zip(lhs) {
            *o = *a + rhs;
        }
    }
    #[inline]
