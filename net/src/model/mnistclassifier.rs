use crate::model::{BatchNorm2D, Conv2D, Dense, Dropout, InitScheme, MaxPool2D, Relu, Softmax};
...
impl MnistCNN {
    ...
    pub fn forward(&mut self, x: &Array4<f32>) -> (Array4<f32>, Array2<f32>, Array2<f32>) {
        let x = self
            .p1
            .forward(&self.r1.forward(&self.bn1.forward(&self.c1.forward(x))));
        let x = self
            .p2
            .forward(&self.r2.forward(&self.bn2.forward(&self.c2.forward(&x))));
    ...
    pub fn train_batch(&mut self, x: &Array4<f32>, y: &Array2<f32>, lr: f32) -> f32 {
        ...
        let grad = self.c2.backward(
            &self
                .r2
                .backward(&self.bn2.backward(&self.p2.backward(&grad))),
        );
        ...
        let _ = self.c1.backward(
            &self
                .r1
                .backward(&self.bn1.backward(&self.p1.backward(&grad))),
        );
