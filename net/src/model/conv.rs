...
impl Conv2D {
    pub fn new(
        ...
        let weights = match scheme {
            ...
            InitScheme::Constant(c) => {
                Array::from_elem((out_channels, in_channels, kernel_size, kernel_size), c)
            }

            InitScheme::RandomUniform(scale) => Array::from_shape_fn(
                (out_channels, in_channels, kernel_size, kernel_size),
    ...
    pub fn forward(&mut self, input: &Array4<f32>) -> Array4<f32> {
        ...
        //alloc output
        let mut output = Mutex::new(Array4::<f32>::zeros((
            batch,
            self.out_channels,
            out_h,
            out_w,
                        ...
                        for ic in 0..self.in_channels {
                            for kh in 0..self.kernel_size {
                                for kw in 0..self.kernel_size {
                                    ...
                                    sum += input[[b, ic, ih, iw]] * self.weights[[c, ic, kh, kw]];
                                }
                            }
                        }
    ...
    pub fn backward(&mut self, grad_output: &Array4<f32>) -> Array4<f32> {
        let input = self
            .input
            .as_ref()
            .expect("Conv2D: forward() must be called before backward()");
                        ...
                        for ic in 0..self.in_channels {
                            for kh in 0..self.kernel_size {
                                for kw in 0..self.kernel_size {
                                    ...
                                    grad_input_padded[[b, ic, h, w]] +=
                                        self.weights[[oc, ic, kh, kw]] * val;
                                }
                            }
                        }
                    }
        ...
        for c in 0..channels {
            ...
            for b in 0..batch_size {
                for h in 0..height {
                    for w in 0..width {
                        output[[b, c, h, w]] =
                            self.gamma[c] * (input[[b, c, h, w]] - mean) / std + self.beta[c];
                    }
                }
            }
        }
        ...
        for c in 0..channels {
            ...
            for b in 0..batch_size {
                for h in 0..height {
                    for w in 0..width {
                        let x_hat = (grad_output[[b, c, h, w]] - mean) / std;
                        grad_input[[b, c, h, w]] = gamma
                            * (grad_output[[b, c, h, w]]
                                - grad_gamma / n as f32
                                - x_hat * grad_x_hat[[b, 0, h, w]] / n as f32)
                            / std;
                    }
                }
            }
                    ...
                    for j in 0..w {
                        for di in 0..self.stride {
                            for dj in 0..self.stride {
                                output[[batch, ch, i * self.stride + di, j * self.stride + dj]] =
                                    input[[batch, ch, i, j]];
                            }
                        }
                    }
    ...
    pub fn forward(&mut self, prev: &Array4<f32>, from: &Array4<f32>) -> Array4<f32> {
        ...
        if prev.shape()[1] != from.shape()[1] {
            if self.conv.is_none() {
                println!(
                    "  Creating channel-matching conv: {} -> {}",
                    from.shape()[1],
                    prev.shape()[1]
                );
